...
static void parse_operand(TCCState *s1, Operand *op)
{
    ...
    if (s1->tok == '%') {
        next();
        if (s1->tok >= TOK_ASM_al && s1->tok <= TOK_ASM_db7) {
            reg = s1->tok - TOK_ASM_al;
            ...
            if ((op->type & OP_REG) && op->reg == TREG_EAX)
                ...
                op->type |= OP_DX;
        } else if (s1->tok >= TOK_ASM_dr0 && s1->tok <= TOK_ASM_dr7) {
            op->type = OP_DB;
            op->reg = s1->tok - TOK_ASM_dr0;
        } else if (s1->tok >= TOK_ASM_es && s1->tok <= TOK_ASM_gs) {
            op->type = OP_SEG;
            op->reg = s1->tok - TOK_ASM_es;
        } else if (s1->tok == TOK_ASM_st) {
            op->type = OP_ST;
            op->reg = 0;
            next();
            if (s1->tok == '(') {
...
{
    if (pe->sym)
        greloc(tcc_state->cur_text_section, pe->sym, tcc_state->ind, R_386_32);
    gen_le32(pe->v);
}
...
static void gen_disp32(ExprValue *pe)
{
    ...
    if (sym) {
        if (sym->r == tcc_state->cur_text_section->sh_num) {
            ...
            gen_le32(pe->v + (long)sym->next - tcc_state->ind - 4);
        } else {
            greloc(tcc_state->cur_text_section, sym, tcc_state->ind, R_386_PC32);
            gen_le32(pe->v - 4);
        }
    } else {
        /* put an empty PC32 relocation */
        put_elf_reloc(tcc_state->symtab_section, tcc_state->cur_text_section, 
                      tcc_state->ind, R_386_PC32, 0);
        gen_le32(pe->v - 4);
    }
}
...
static void asm_opcode(TCCState *s1, int opcode)
{
    ...
    if (pa->instr_type & OPC_SHORTJMP) {
        ...
        if (!sym)
            goto no_short_jump;
        if (sym->r != s1->cur_text_section->sh_num)
            goto no_short_jump;
        jmp_disp = ops[0].e.v + (long)sym->next - s1->ind - 2;
