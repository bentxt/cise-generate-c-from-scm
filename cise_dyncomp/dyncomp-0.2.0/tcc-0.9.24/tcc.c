...
struct TCCState {
    ...
    */
    ...
    CType func_vt; /* current function return type (used by return
                         instruction) */
    int func_vc;
    int func_ind; /* debug last line number and pc */
    int tok_ident;
...
/* Added by KOGURO, Naoki */
static struct TCCStateStack {
    TCCState *state;
    struct TCCStateStack *prev;
} *tcc_state_stack = NULL;
                            ...
                            int can_add_underscore)
{
    ...
    if (!sym->c) {
        ...
        esym = &((Elf32_Sym *)tcc_state->symtab_section->data)[sym->c];
        esym->st_value = 0;
...
static void greloc(Section *s, Sym *sym, unsigned long offset, int type)
{
    ...
    put_elf_reloc(tcc_state->symtab_section, s, offset, type, sym->c);
}
...
void error1(TCCState *s1, int is_warning, const char *fmt, va_list ap)
{
    ...
    if (s1->file) {
        ...
        if (s1->file->line_num > 0) {
            strcat_printf(buf, sizeof(buf), 
                          "%s:%d: ", s1->file->filename, s1->file->line_num);
        } else {
            strcat_printf(buf, sizeof(buf),
                          "%s: ", s1->file->filename);
        }
...
static TokenSym *tok_alloc_new(TokenSym **pts, const char *str, int len)
{
    ...
    if (tcc_state->tok_ident >= SYM_FIRST_ANOM) 
        tcc_error("memory full");

    /* expand token table if needed */
    i = tcc_state->tok_ident - TOK_IDENT;
    if ((i % TOK_ALLOC_INCR) == 0) {
        ptable = tcc_realloc(tcc_state->table_ident, (i + TOK_ALLOC_INCR) * sizeof(TokenSym *));
        if (!ptable)
            tcc_error("memory full");
        tcc_state->table_ident = ptable;
    }

    ts = tcc_malloc(sizeof(TokenSym) + len);
    tcc_state->table_ident[i] = ts;
    ts->tok = tcc_state->tok_ident++;
    ts->sym_define = NULL;
    ts->sym_label = NULL;
    ts->sym_struct = NULL;
...
char *get_tok_str(int v, CValue *cv)
{
    ...
    default:
        ...
        addv:
            *p++ = v;
            *p = '\0';
        } else if (v < tcc_state->tok_ident) {
            return tcc_state->table_ident[v - TOK_IDENT]->str;
...
{
    v -= TOK_IDENT;
    if ((unsigned)v >= (unsigned)(tcc_state->tok_ident - TOK_IDENT))
        return NULL;
    return tcc_state->table_ident[v]->sym_struct;
}
...
static inline Sym *sym_find(int v)
{
    ...
    if ((unsigned)v >= (unsigned)(tcc_state->tok_ident - TOK_IDENT))
        return NULL;
    return tcc_state->table_ident[v]->sym_identifier;
}
...
static Sym *sym_push(int v, CType *type, int r, int c)
{
    ...
    if (tcc_state->local_stack)
        ps = &tcc_state->local_stack;
    else
        ps = &tcc_state->global_stack;
    ...
    if (!(v & SYM_FIELD) && (v & ~SYM_STRUCT) < SYM_FIRST_ANOM) {
        ...
        ts = tcc_state->table_ident[(v & ~SYM_STRUCT) - TOK_IDENT];
        if (v & SYM_STRUCT)
            ps = &ts->sym_struct;
        else
...
static Sym *global_identifier_push(int v, int t, int c)
{
    ...
    if (v < SYM_FIRST_ANOM) {
        ps = &tcc_state->table_ident[v - TOK_IDENT]->sym_identifier;
...
{
    return tcc_peekc_slow(tcc_state->file);
}

/* read next char from current input file and handle end of input buffer */
static inline void inp(void)
{
    tcc_state->ch = *(++(tcc_state->file->buf_ptr));
    /* end of buffer/file handling */
    if (tcc_state->ch == CH_EOB)
        tcc_state->ch = handle_eob();
}
...
static int handle_stray_noerror(void)
{
    while (tcc_state->ch == '\\') {
        inp();
        if (tcc_state->ch == '\n') {
            tcc_state->file->line_num++;
            inp();
        } else if (tcc_state->ch == '\r') {
            inp();
            if (tcc_state->ch != '\n')
                goto fail;
            tcc_state->file->line_num++;
...
static int handle_stray1(uint8_t *p)
{
    ...
    if (p >= tcc_state->file->buf_end) {
        tcc_state->file->buf_ptr = p;
        c = handle_eob();
        p = tcc_state->file->buf_ptr;
        if (c == '\\')
            goto parse_stray;
    } else {
    parse_stray:
    ...
    redo_no_start:
        ...
        case '#':
            ...
            if (start_of_line) {
                ...
                p = tcc_state->file->buf_ptr;
                if (a == 0 && 
                    (tcc_state->tok == TOK_ELSE || tcc_state->tok == TOK_ELIF || tcc_state->tok == TOK_ENDIF))
                    goto the_end;
                if (tcc_state->tok == TOK_IF || tcc_state->tok == TOK_IFDEF || tcc_state->tok == TOK_IFNDEF)
                    a++;
                else if (tcc_state->tok == TOK_ENDIF)
                    a--;
                else if( tcc_state->tok == TOK_ERROR || tcc_state->tok == TOK_WARNING)
                    in_warn_or_error = 1;
            }
            break;
_default:
...
{
    s->line_num = tcc_state->file->line_num;
    s->macro_ptr = tcc_state->macro_ptr;
    s->tok = tcc_state->tok;
    s->tokc = tcc_state->tokc;
}

/* restore parse state from 's' */
void restore_parse_state(ParseState *s)
{
    tcc_state->file->line_num = s->line_num;
    tcc_state->macro_ptr = s->macro_ptr;
    tcc_state->tok = s->tok;
    tcc_state->tokc = s->tokc;
}
...
static void tok_str_add_tok(TokenString *s)
{
    ...
    if (tcc_state->file->line_num != s->last_line_num) {
        s->last_line_num = tcc_state->file->line_num;
        cval.i = s->last_line_num;
        tok_str_add2(s, TOK_LINENUM, &cval);
    }
    tok_str_add2(s, tcc_state->tok, &tcc_state->tokc);
}
...
static inline void define_push(int v, int macro_type, int *str, Sym *first_arg)
{
    ...
    s = sym_push2(&tcc_state->define_stack, v, macro_type, (int)str);
    s->next = first_arg;
    tcc_state->table_ident[v - TOK_IDENT]->sym_define = s;
}
...
static void define_undef(Sym *s)
{
    int v;
    v = s->v;
    if (v >= TOK_IDENT && v < tcc_state->tok_ident)
        tcc_state->table_ident[v - TOK_IDENT]->sym_define = NULL;
    s->v = 0;
}
...
static inline Sym *define_find(int v)
{
    ...
    if ((unsigned)v >= (unsigned)(tcc_state->tok_ident - TOK_IDENT))
        return NULL;
    return tcc_state->table_ident[v]->sym_define;
}
...
static Sym *label_find(int v)
{
    ...
    if ((unsigned)v >= (unsigned)(tcc_state->tok_ident - TOK_IDENT))
        return NULL;
    return tcc_state->table_ident[v]->sym_label;
}
...
static Sym *label_push(Sym **ptop, int v, int flags)
{
    ...
    s->r = flags;
    ps = &tcc_state->table_ident[v - TOK_IDENT]->sym_label;
        ...
        if (s->r == LABEL_DECLARED) {
            ...
            if (s->c) {
                /* define corresponding symbol. A size of
                   1 is put. */
                put_extern_sym(s, tcc_state->cur_text_section, (long)s->next, 1);
            }
        }
        /* remove label */
        tcc_state->table_ident[s->v - TOK_IDENT]->sym_label = s->prev_tok;
        sym_free(s);
    }
    *ptop = slast;
}
...
static int expr_preprocess(void)
{
    ...
    tok_str_new(&str);
    while (tcc_state->tok != TOK_LINEFEED && tcc_state->tok != TOK_EOF) {
        next(); /* do macro subst */
        if (tcc_state->tok == TOK_DEFINED) {
            next_nomacro();
            t = tcc_state->tok;
            if (t == '(') 
                next_nomacro();
            c = define_find(tcc_state->tok) != 0;
            if (t == '(')
                next_nomacro();
            tcc_state->tok = TOK_CINT;
            tcc_state->tokc.i = c;
...
static void parse_define(void)
{
    ...
    v = tcc_state->tok;
    if (v < TOK_IDENT)
        tcc_error("invalid macro name '%s'", get_tok_str(tcc_state->tok, &tcc_state->tokc));
    /* XXX: should check if same macro (ANSI) */
    first = NULL;
    t = MACRO_OBJ;
    /* '(' must be just after macro definition for MACRO_FUNC */
    c = tcc_state->file->buf_ptr[0];
    if (c == '\\')
        c = handle_stray1(tcc_state->file->buf_ptr);
    if (c == '(') {
        next_nomacro();
        next_nomacro();
        ps = &first;
        while (tcc_state->tok != ')') {
            ...
            if (varg == TOK_DOTS) {
                ...
                is_vaargs = 1;
            } else if (tcc_state->tok == TOK_DOTS && gnu_ext) {
                is_vaargs = 1;
                next_nomacro();
            }
            if (varg < TOK_IDENT)
    ...
    while (tcc_state->tok != TOK_LINEFEED && tcc_state->tok != TOK_EOF) {
        tok_str_add2(&str, tcc_state->tok, &tcc_state->tokc);
        next_nomacro();
    }
    tok_str_add(&str, 0);
#ifdef PP_DEBUG
...
static void preprocess(int is_bof)
{
 ...
 redo:
    ...
    case TOK_INCLUDE_NEXT:
            ...
            *q = '\0';
            ...
            if (tcc_state->tok == TOK_STR) {
                while (tcc_state->tok != TOK_LINEFEED) {
                    ...
                    pstrcat(buf, sizeof(buf), (char *)tcc_state->tokc.cstr->data);
                    next();
                }
                c = '\"';
            } else {
                int len;
                while (tcc_state->tok != TOK_LINEFEED) {
                    pstrcat(buf, sizeof(buf), get_tok_str(tcc_state->tok, &tcc_state->tokc));
                    next();
                }
                len = strlen(buf);
        ...
        if (e && define_find(e->ifndef_macro)) {
            ...
            *s1->include_stack_ptr++ = s1->file;
            if (c == '\"') {
                ...
                size = tcc_basename(s1->file->filename) - s1->file->filename;
                if (size > sizeof(buf1) - 1)
                    size = sizeof(buf1) - 1;
                memcpy(buf1, s1->file->filename, size);
                buf1[size] = '\0';
                pstrcat(buf1, sizeof(buf1), buf);
                f = tcc_open(s1, buf1);
                if (f) {
                    if (tcc_state->tok == TOK_INCLUDE_NEXT)
                        tcc_state->tok = TOK_INCLUDE;
                    else
                        goto found;
                }
            }
        ...
        found:
#ifdef INC_DEBUG
            printf("%s: including %s\n", tcc_state->file->filename, buf1);
#endif
            f->inc_type = c;
            pstrcpy(f->inc_filename, sizeof(f->inc_filename), buf);
            tcc_state->file = f;
            tcc_state->tok_flags |= TOK_FLAG_BOF | TOK_FLAG_BOL;
            tcc_state->ch = tcc_state->file->buf_ptr[0];
            goto the_end;
        }
        break;
...
#endif
                tcc_state->file->ifndef_macro = tcc_state->tok;
            }
        }
        c = (define_find(tcc_state->tok) != 0) ^ c;
    do_if:
    ...
    case TOK_LINE:
        next();
        if (tcc_state->tok != TOK_CINT)
            tcc_error("#line");
        tcc_state->file->line_num = tcc_state->tokc.i - 1; /* the line number will be incremented after */
        next();
        if (tcc_state->tok != TOK_LINEFEED) {
            if (tcc_state->tok != TOK_STR)
                tcc_error("#line");
            pstrcpy(tcc_state->file->filename, sizeof(tcc_state->file->filename), 
                    (char *)tcc_state->tokc.cstr->data);
        }
        break;
    case TOK_ERROR:
    case TOK_WARNING:
    ...
    case TOK_PRAGMA:
        ...
        break;
    default:
        if (tcc_state->tok == TOK_LINEFEED || tcc_state->tok == '!' || tcc_state->tok == TOK_CINT) {
            /* '!' is ignored to allow C scripts. numbers are ignored
               to emulate cpp behaviour */
        } else {
            if (!(saved_parse_flags & PARSE_FLAG_ASM_COMMENTS))
                warning("Ignoring unknown preprocessing directive #%s", get_tok_str(tcc_state->tok, &tcc_state->tokc));
        }
        break;
    }
...
void parse_number(const char *p)
{
    ...
    *q++ = t;
    ...
    while (1) {
        ...
        if (t >= b)
            break;
        if (q >= tcc_state->token_buf + STRING_MAX_SIZE) {
        num_too_long:
    ...
    if (ch == '.' ||
        ...
        if (b != 10) {
            ...
            *q = '\0';
            ...
            if (t == 'F') {
                ch = *p++;
                tcc_state->tok = TOK_CFLOAT;
                /* float : should handle overflow */
                tcc_state->tokc.f = (float)d;
            } else if (t == 'L') {
                ch = *p++;
                tcc_state->tok = TOK_CLDOUBLE;
                /* XXX: not large enough */
                tcc_state->tokc.ld = (long double)d;
            } else {
                tcc_state->tok = TOK_CDOUBLE;
                tcc_state->tokc.d = d;
            }
        } else {
            /* decimal floats */
            ...
            *q = '\0';
            ...
            if (t == 'F') {
                ch = *p++;
                tcc_state->tok = TOK_CFLOAT;
                tcc_state->tokc.f = strtof(tcc_state->token_buf, NULL);
            } else if (t == 'L') {
                ch = *p++;
                tcc_state->tok = TOK_CLDOUBLE;
                tcc_state->tokc.ld = strtold(tcc_state->token_buf, NULL);
            } else {
                tcc_state->tok = TOK_CDOUBLE;
                tcc_state->tokc.d = strtod(tcc_state->token_buf, NULL);
            }
        }
    } else {
        ...
        *q = '\0';
        ...
        if ((n & 0xffffffff00000000LL) != 0) {
            if ((n >> 63) != 0)
                tcc_state->tok = TOK_CULLONG;
            else
                tcc_state->tok = TOK_CLLONG;
        } else if (n > 0x7fffffff) {
            tcc_state->tok = TOK_CUINT;
        } else {
            tcc_state->tok = TOK_CINT;
        }
        lcount = 0;
        ...
        for(;;) {
            ...
            if (t == 'L') {
                ...
                if (lcount == 2) {
                    if (tcc_state->tok == TOK_CINT)
                        tcc_state->tok = TOK_CLLONG;
                    else if (tcc_state->tok == TOK_CUINT)
                        tcc_state->tok = TOK_CULLONG;
                }
                ch = *p++;
            } else if (t == 'U') {
                ...
                if (tcc_state->tok == TOK_CINT)
                    tcc_state->tok = TOK_CUINT;
                else if (tcc_state->tok == TOK_CLLONG)
                    tcc_state->tok = TOK_CULLONG;
                ch = *p++;
            } else {
                break;
        ...
        if (tcc_state->tok == TOK_CINT || tcc_state->tok == TOK_CUINT)
            tcc_state->tokc.ui = n;
        else
            tcc_state->tokc.ull = n;
    }
}
...
static inline void next_nomacro1(void)
{
 ...
 redo_no_start:
...
#endif
                    add_cached_include(s1, s1->file->inc_type, s1->file->inc_filename,
                                       s1->file->ifndef_macro_saved);
                }

                /* pop include stack */
                tcc_close(s1->file);
                s1->include_stack_ptr--;
                s1->file = *s1->include_stack_ptr;
                p = s1->file->buf_ptr;
                goto redo_no_start;
            }
        }
    ...
    case '\n':
        tcc_state->file->line_num++;
        tcc_state->tok_flags |= TOK_FLAG_BOL;
        p++;
        if (0 == (tcc_state->parse_flags & PARSE_FLAG_LINEFEED))
            goto redo_no_start;
        tcc_state->tok = TOK_LINEFEED;
        goto keep_tok_flags;

    case '#':
        /* XXX: simplify */
        PEEKC(c, p);
        if ((tcc_state->tok_flags & TOK_FLAG_BOL) && 
            (tcc_state->parse_flags & PARSE_FLAG_PREPROCESS)) {
            tcc_state->file->buf_ptr = p;
            preprocess(tcc_state->tok_flags & TOK_FLAG_BOF);
            p = tcc_state->file->buf_ptr;
            goto redo_no_start;
        } else {
            if (c == '#') {
                p++;
                tcc_state->tok = TOK_TWOSHARPS;
            } else {
                if (tcc_state->parse_flags & PARSE_FLAG_ASM_COMMENTS) {
    ...
    parse_ident_fast:
        ...
        token_found: ;
            ...
            /* slower case */
            cstr_reset(&tcc_state->tokcstr);

            while (p1 < p) {
                cstr_ccat(&tcc_state->tokcstr, *p1);
                p1++;
            }
            p--;
            PEEKC(c, p);
        parse_ident_slow:
            while (isidnum_table[c]) {
                cstr_ccat(&tcc_state->tokcstr, c);
                PEEKC(c, p);
            }
            ts = tok_alloc(tcc_state->tokcstr.data, tcc_state->tokcstr.size);
        }
        tcc_state->tok = ts->tok;
        break;
    case 'L':
        t = p[1];
        if (t != '\\' && t != '\'' && t != '\"') {
    ...
    parse_num:
        ...
        cstr_ccat(&tcc_state->tokcstr, '\0');
        tcc_state->tokc.cstr = &tcc_state->tokcstr;
        tcc_state->tok = TOK_PPNUM;
        break;
    case '.':
    ...
    str_const:
        {
            ...
            cstr_reset(&tcc_state->tokcstr);
            parse_escape_string(&tcc_state->tokcstr, str.data, is_long);
            cstr_free(&str);
            ...
            if (sep == '\'') {
                ...
                if (tcc_state->tokcstr.size <= char_size)
                    tcc_error("empty character constant");
                if (tcc_state->tokcstr.size > 2 * char_size)
                    warning("multi-character character constant");
                if (!is_long) {
                    tcc_state->tokc.i = *(int8_t *)tcc_state->tokcstr.data;
                    tcc_state->tok = TOK_CCHAR;
                } else {
                    tcc_state->tokc.i = *(nwchar_t *)tcc_state->tokcstr.data;
                    tcc_state->tok = TOK_LCHAR;
                }
            } else {
                tcc_state->tokc.cstr = &tcc_state->tokcstr;
                if (!is_long)
                    tcc_state->tok = TOK_STR;
                else
                    tcc_state->tok = TOK_LSTR;
            }
        }
        break;
    ...
    case '<':
        ...
        if (c == '=') {
            p++;
            tcc_state->tok = TOK_LE;
        } else if (c == '<') {
            PEEKC(c, p);
            if (c == '=') {
                p++;
                tcc_state->tok = TOK_A_SHL;
            } else {
                tcc_state->tok = TOK_SHL;
            }
        } else {
            tcc_state->tok = TOK_LT;
        }
        break;
    ...
    case '>':
        ...
        if (c == '=') {
            p++;
            tcc_state->tok = TOK_GE;
        } else if (c == '>') {
            PEEKC(c, p);
            if (c == '=') {
                p++;
                tcc_state->tok = TOK_A_SAR;
            } else {
                tcc_state->tok = TOK_SAR;
            }
        } else {
            tcc_state->tok = TOK_GT;
        }
        break;
    ...
    case '&':
        ...
        if (c == '&') {
            p++;
            tcc_state->tok = TOK_LAND;
        } else if (c == '=') {
            p++;
            tcc_state->tok = TOK_A_AND;
        } else {
            tcc_state->tok = '&';
        }
        break;
    ...
    case '|':
        ...
        if (c == '|') {
            p++;
            tcc_state->tok = TOK_LOR;
        } else if (c == '=') {
            p++;
            tcc_state->tok = TOK_A_OR;
        } else {
            tcc_state->tok = '|';
        }
        break;
    ...
    case '+':
        ...
        if (c == '+') {
            p++;
            tcc_state->tok = TOK_INC;
        } else if (c == '=') {
            p++;
            tcc_state->tok = TOK_A_ADD;
        } else {
            tcc_state->tok = '+';
        }
        break;
    ...
    case '-':
        ...
        if (c == '-') {
            p++;
            tcc_state->tok = TOK_DEC;
        } else if (c == '=') {
            p++;
            tcc_state->tok = TOK_A_SUB;
        } else if (c == '>') {
            p++;
            tcc_state->tok = TOK_ARROW;
        } else {
            tcc_state->tok = '-';
        }
        break;
    ...
    tcc_state->tok_flags = 0;
keep_tok_flags:
    tcc_state->file->buf_ptr = p;
#if defined(PARSE_DEBUG)
    printf("token = %s\n", get_tok_str(tcc_state->tok, &tcc_state->tokc));
#endif
}
...
static void next_nomacro(void)
{
    if (tcc_state->macro_ptr) {
    redo:
        tcc_state->tok = *tcc_state->macro_ptr;
        if (tcc_state->tok) {
            TOK_GET(tcc_state->tok, tcc_state->macro_ptr, tcc_state->tokc);
            if (tcc_state->tok == TOK_LINENUM) {
                tcc_state->file->line_num = tcc_state->tokc.i;
                goto redo;
            }
        }
    } else {
                           ...
                           Sym **nested_list, Sym *s, struct macro_level **can_read_stream)
{
    ...
    /* special macros */
    if (tcc_state->tok == TOK___LINE__) {
        snprintf(buf, sizeof(buf), "%d", tcc_state->file->line_num);
        cstrval = buf;
        t1 = TOK_PPNUM;
        goto add_cstr1;
    } else if (tcc_state->tok == TOK___FILE__) {
        cstrval = tcc_state->file->filename;
        goto add_cstr;
    } else if (tcc_state->tok == TOK___DATE__ || tcc_state->tok == TOK___TIME__) {
        time_t ti;
        struct tm *tm;

        time(&ti);
        tm = localtime(&ti);
        if (tcc_state->tok == TOK___DATE__) {
    ...
    add_cstr1:
        ...
        redo:
            if (tcc_state->macro_ptr) {
                ...
                tcc_state->ch = tcc_state->file->buf_ptr[0];
                while (is_space(tcc_state->ch) || tcc_state->ch == '\n')
                    cinp();
                t = tcc_state->ch;
            }
            if (t != '(') /* no macro subst */
...
static inline int *macro_twosharps(const int *macro_str)
{
    ...
    tcc_state->macro_ptr = (int *)macro_str;
    for(;;) {
        while (*tcc_state->macro_ptr == TOK_TWOSHARPS) {
            tcc_state->macro_ptr++;
            macro_ptr1 = tcc_state->macro_ptr;
            t = *tcc_state->macro_ptr;
            if (t) {
                TOK_GET(t, tcc_state->macro_ptr, cval);
                /* We concatenate the two tokens if we have an
                   identifier or a preprocessing number */
                cstr_reset(&cstr);
                p1 = get_tok_str(tcc_state->tok, &tcc_state->tokc);
                cstr_cat(&cstr, p1);
                p2 = get_tok_str(t, &cval);
                cstr_cat(&cstr, p2);
                cstr_ccat(&cstr, '\0');
                
                if ((tcc_state->tok >= TOK_IDENT || tcc_state->tok == TOK_PPNUM) && 
                    ...
                    if (tcc_state->tok == TOK_PPNUM) {
                        ...
                        tcc_state->tok = ts->tok; /* modify current token */
                    }
                } else {
                    ...
                    error_pasting:
                        ...
                        tok_str_add2(&macro_str1, tcc_state->tok, &tcc_state->tokc);
    ...
    tcc_state->macro_ptr = (int *)saved_macro_ptr;
    cstr_free(&cstr);
    ...
    while (1) {
        ...
        if (s != NULL) {
            ...
            if (sym_find2(*nested_list, t))
                goto no_subst;
            ml.p = tcc_state->macro_ptr;
            if (can_read_stream)
                ml.prev = *can_read_stream, *can_read_stream = &ml;
            tcc_state->macro_ptr = (int *)ptr;
            tcc_state->tok = t;
            ret = macro_subst_tok(tok_str, nested_list, s, can_read_stream);
            ptr = (int *)tcc_state->macro_ptr;
            tcc_state->macro_ptr = ml.p;
 ...
 redo:
    ...
    if (!tcc_state->macro_ptr) {
        ...
        if (tcc_state->tok >= TOK_IDENT &&
            ...
            if (s) {
                ...
                if (macro_subst_tok(&str, &nested_list, s, &ml) == 0) {
                    ...
                    tok_str_add(&str, 0);
                    tcc_state->macro_ptr = str.str;
                    tcc_state->macro_ptr_allocated = str.str;
                    goto redo;
                }
            }
        }
        ...
        if (tcc_state->tok == 0) {
            ...
            if (tcc_state->unget_buffer_enabled) {
                tcc_state->macro_ptr = tcc_state->unget_saved_macro_ptr;
                tcc_state->unget_buffer_enabled = 0;
            } else {
                /* end of macro string: free it */
                tok_str_free(tcc_state->macro_ptr_allocated);
                tcc_state->macro_ptr = NULL;
            }
            goto redo;
        }
    ...
    if (tcc_state->tok == TOK_PPNUM &&
        (tcc_state->parse_flags & PARSE_FLAG_TOK_NUM)) {
        parse_number((char *)tcc_state->tokc.cstr->data);
    }
}
...
static inline void unget_tok(int last_tok)
{
    int i, n;
    int *q;
    tcc_state->unget_saved_macro_ptr = tcc_state->macro_ptr;
    tcc_state->unget_buffer_enabled = 1;
    q = tcc_state->unget_saved_buffer;
    tcc_state->macro_ptr = q;
    *q++ = tcc_state->tok;
    n = tok_ext_size(tcc_state->tok) - 1;
    for(i=0;i<n;i++)
        *q++ = tcc_state->tokc.tab[i];
    *q = 0; /* end of token string */
    tcc_state->tok = last_tok;
}
    ...
    for(p=vstack;p<=vtop;p++) {
        if ((p->r & VT_VALMASK) == r ||
            ...
            if (!saved) {
                ...
                tcc_state->loc = (tcc_state->loc - size) & -align;
                sv.type.t = type->t;
                sv.r = VT_LOCAL | VT_LVAL;
                sv.c.ul = tcc_state->loc;
                store(r, &sv);
#ifdef TCC_TARGET_I386
...
int gv(int rc)
{
    ...
    if (vtop->type.t & VT_BITFIELD) {
        ...
        if (is_float(vtop->type.t) && 
            ...
            offset = (tcc_state->data_section->data_offset + align - 1) & -align;
            tcc_state->data_section->data_offset = offset;
            ...
            for(i=0;i<size;i++)
                ptr[i] = vtop->c.tab[i];
            sym = get_sym_ref(&vtop->type, tcc_state->data_section, offset, size << 2);
...
void vpop(void)
{
    ...
    if (v == TREG_ST0 && !tcc_state->nocode_wanted) {
        o(0xd9dd); /* fstp %st(1) */
    } else
#endif
...
void gen_op(int op)
{
    ...
    if (bt1 == VT_PTR || bt2 == VT_PTR) {
        ...
        if (bt1 == VT_PTR && bt2 == VT_PTR) {
            ...
            if (do_bounds_check && !tcc_state->const_wanted) {
...
static void gen_cast(CType *type)
{
    ...
    if (sbt != dbt && !tcc_state->nocode_wanted) {
        sf = is_float(sbt);
        df = is_float(dbt);
...
static void parse_attribute(AttributeDef *ad)
{
    ...
    while (tcc_state->tok == TOK_ATTRIBUTE1 || tcc_state->tok == TOK_ATTRIBUTE2) {
    next();
    skip('(');
    skip('(');
    while (tcc_state->tok != ')') {
        ...
        case TOK_SECTION2:
            ...
            if (tcc_state->tok != TOK_STR)
                expect("section name");
            ad->section = find_section(tcc_state, (char *)tcc_state->tokc.cstr->data);
            next();
            skip(')');
            break;
        case TOK_ALIGNED1:
            ...
            if (tcc_state->tok == '(') {
                ...
                do {
                    if (tcc_state->tok == '(') 
                        ...
                        parenthesis--;
                    next();
                } while (parenthesis && tcc_state->tok != -1);
            }
            break;
        }
        if (tcc_state->tok != ',')
...
static void struct_decl(CType *type, int u)
{
    ...
    a = tcc_state->tok; /* save decl type */
    next();
    if (tcc_state->tok != '{') {
        v = tcc_state->tok;
        next();
        ...
        if (a == TOK_ENUM) {
            ...
            while (tcc_state->tok != '}') {
                ...
                while (1) {
                    ...
                    if (tcc_state->tok == ';' || tcc_state->tok == TOK_EOF)
                        break;
                    skip(',');
                }
                skip(';');
        ...
        if (tcc_state->tok != ')') {
            for(;;) {
                ...
                if (l != FUNC_OLD) {
                    ...
                    l = FUNC_NEW;
                    if ((pt.t & VT_BTYPE) == VT_VOID && tcc_state->tok == ')')
                        break;
                ...
                *plast = s;
                ...
                if (tcc_state->tok == ')')
                    break;
                skip(',');
                if (l == FUNC_NEW && tcc_state->tok == TOK_DOTS) {
                    l = FUNC_ELLIPSIS;
                    next();
                    break;
                }
...
static void type_decl(CType *type, AttributeDef *ad, int *v, int td)
{
    ...
    if (tcc_state->tok == TOK_ATTRIBUTE1 || tcc_state->tok == TOK_ATTRIBUTE2)
        parse_attribute(ad);
    ...
    if (tcc_state->tok == '(') {
        ...
        /* type identifier */
        if (tcc_state->tok >= TOK_IDENT && (td & TYPE_DIRECT)) {
            *v = tcc_state->tok;
            next();
        } else {
            if (!(td & TYPE_ABSTRACT))
...
static void indir(void)
{
    ...
    if ((vtop->r & VT_LVAL) && !tcc_state->nocode_wanted)
        gv(RC_INT);
...
static void vpush_tokc(int t)
{
    CType type;
    type.t = t;
    vsetc(&type, VT_CONST, &tcc_state->tokc);
}
...
static void unary(void)
{
 ...
 tok_next:
    ...
    case TOK_builtin_constant_p:
        {
            ...
            next();
            skip('(');
            saved_nocode_wanted = tcc_state->nocode_wanted;
            tcc_state->nocode_wanted = 1;
            gexpr();
            res = (vtop->r & (VT_VALMASK | VT_LVAL | VT_SYM)) == VT_CONST;
            vpop();
            tcc_state->nocode_wanted = saved_nocode_wanted;
            skip(')');
            vpushi(res);
        }
        break;
    ...
    case TOK_LAND:
        ...
        if (tcc_state->tok < TOK_UIDENT)
            expect("label identifier");
        s = label_find(tcc_state->tok);
        if (!s) {
            s = label_push(&tcc_state->global_label_stack, tcc_state->tok, LABEL_FORWARD);
        } else {
            if (s->r == LABEL_DECLARED)
                s->r = LABEL_FORWARD;
        }
    ...
    tok_identifier:
        ...
        if ((s->type.t & (VT_STATIC | VT_INLINE | VT_BTYPE)) ==
            ...
            if (!s->c)
                put_extern_sym(s, tcc_state->text_section, 0, 0);
            r = VT_SYM | VT_CONST;
        } else {
            r = s->r;
        }
    ...
    /* post operations */
    while (1) {
        if (tcc_state->tok == TOK_INC || tcc_state->tok == TOK_DEC) {
            inc(1, tcc_state->tok);
            next();
        } else if (tcc_state->tok == '.' || tcc_state->tok == TOK_ARROW) {
            /* field */ 
            if (tcc_state->tok == TOK_ARROW) 
                indir();
            test_lvalue();
            gaddrof();
            next();
            ...
            s = vtop->type.ref;
            /* find field */
            tcc_state->tok |= SYM_FIELD;
            while ((s = s->next) != NULL) {
                if (s->v == tcc_state->tok)
                    break;
            }
            if (!s)
                tcc_error("field not found: %s",  get_tok_str(tcc_state->tok & ~SYM_FIELD, NULL));
...
static void uneq(void)
{
    ...
    unary();
    if (tcc_state->tok == '=' ||
        (tcc_state->tok >= TOK_A_MOD && tcc_state->tok <= TOK_A_DIV) ||
        tcc_state->tok == TOK_A_XOR || tcc_state->tok == TOK_A_OR ||
        tcc_state->tok == TOK_A_SHL || tcc_state->tok == TOK_A_SAR) {
        test_lvalue();
        t = tcc_state->tok;
        next();
        if (t == '=') {
...
static void expr_prod(void)
{
    ...
    uneq();
    while (tcc_state->tok == '*' || tcc_state->tok == '/' || tcc_state->tok == '%') {
        t = tcc_state->tok;
        next();
        uneq();
        gen_op(t);
...
static void expr_sum(void)
{
    ...
    expr_prod();
    while (tcc_state->tok == '+' || tcc_state->tok == '-') {
        t = tcc_state->tok;
        next();
        expr_prod();
        gen_op(t);
...
static void expr_shift(void)
{
    ...
    expr_sum();
    while (tcc_state->tok == TOK_SHL || tcc_state->tok == TOK_SAR) {
        t = tcc_state->tok;
        next();
        expr_sum();
        gen_op(t);
...
static void expr_cmp(void)
{
    ...
    expr_shift();
    while ((tcc_state->tok >= TOK_ULE && tcc_state->tok <= TOK_GT) ||
           tcc_state->tok == TOK_ULT || tcc_state->tok == TOK_UGE) {
        t = tcc_state->tok;
        next();
        expr_shift();
        gen_op(t);
...
static void expr_cmpeq(void)
{
    ...
    expr_cmp();
    while (tcc_state->tok == TOK_EQ || tcc_state->tok == TOK_NE) {
        t = tcc_state->tok;
        next();
        expr_cmp();
        gen_op(t);
...
static void unary_type(CType *type)
{
    ...
    a = tcc_state->nocode_wanted;
    tcc_state->nocode_wanted = 1;
    unary();
    *type = vtop->type;
    vpop();
    tcc_state->nocode_wanted = a;
}
...
{
    int a;
    a = tcc_state->const_wanted;
    tcc_state->const_wanted = 1;
    expr_eq();
    tcc_state->const_wanted = a;
}
                  ...
                  int case_reg, int is_expr)
{
    ...
    if (tcc_state->tok == TOK_IF) {
        ...
        if (c == TOK_ELSE) {
            ...
            gsym(a);
    } else if (tcc_state->tok == TOK_WHILE) {
        next();
        d = tcc_state->ind;
        skip('(');
        gexpr();
        ...
        s = tcc_state->local_stack;
        llabel = tcc_state->local_label_stack;
        /* handle local labels declarations */
        if (tcc_state->tok == TOK_LABEL) {
            next();
            for(;;) {
                if (tcc_state->tok < TOK_UIDENT)
                    expect("label identifier");
                label_push(&tcc_state->local_label_stack, tcc_state->tok, LABEL_DECLARED);
                next();
                if (tcc_state->tok == ',') {
                    next();
                } else {
        ...
        label_pop(&tcc_state->local_label_stack, llabel);
        /* pop locally defined symbols */
        sym_pop(&tcc_state->local_stack, s);
        next();
    } else if (tcc_state->tok == TOK_RETURN) {
        next();
        if (tcc_state->tok != ';') {
            gexpr();
            gen_assign_cast(&tcc_state->func_vt);
            if ((tcc_state->func_vt.t & VT_BTYPE) == VT_STRUCT) {
                ...
                type = tcc_state->func_vt;
                mk_pointer(&type);
                vset(&type, VT_LOCAL | VT_LVAL, tcc_state->func_vc);
                indir();
                vswap();
...
#endif
            } else if (is_float(tcc_state->func_vt.t)) {
                gv(RC_FRET);
            } else {
                gv(RC_IRET);
            }
        ...
        skip(';');
        tcc_state->rsym = gjmp(tcc_state->rsym); /* jmp */
    } else if (tcc_state->tok == TOK_BREAK) {
        /* compute jump */
        if (!bsym)
        ...
        *bsym = gjmp(*bsym);
        next();
        skip(';');
    } else if (tcc_state->tok == TOK_CONTINUE) {
        /* compute jump */
        if (!csym)
    ...
    if (tcc_state->tok == TOK_CASE) {
        ...
        v1 = expr_const();
        v2 = v1;
        if (gnu_ext && tcc_state->tok == TOK_DOTS) {
            next();
            v2 = expr_const();
            if (v2 < v1)
    ...
    if (tcc_state->tok == TOK_DEFAULT) {
        ...
        *def_sym = tcc_state->ind;
        is_expr = 0;
        goto block_after_label;
    } else
    if (tcc_state->tok == TOK_GOTO) {
        next();
        if (tcc_state->tok == '*' && gnu_ext) {
            /* computed goto */
            next();
            ...
            if ((vtop->type.t & VT_BTYPE) != VT_PTR)
                expect("pointer");
            ggoto();
        } else if (tcc_state->tok >= TOK_UIDENT) {
            s = label_find(tcc_state->tok);
        ...
        skip(';');
    } else if (tcc_state->tok == TOK_ASM1 || tcc_state->tok == TOK_ASM2 || tcc_state->tok == TOK_ASM3) {
        asm_instr();
    } else {
        b = is_label();
        if (b) {
            ...
            if (s) {
                ...
                s = label_push(&tcc_state->global_label_stack, b, LABEL_DEFINED);
            }
            s->next = (void *)tcc_state->ind;
                            ...
                            int size_only)
{
    ...
    while (tcc_state->tok == '[' || tcc_state->tok == '.') {
        if (tcc_state->tok == '[') {
            ...
            if (tcc_state->tok == TOK_DOTS && gnu_ext) {
                next();
                      ...
                      int v, int expr_type)
{
    ...
    case EXPR_CONST:
        ...
        saved_global_expr = tcc_state->global_expr;
        tcc_state->global_expr = 1;
        expr_const1();
        tcc_state->global_expr = saved_global_expr;
                             ...
                             int first, int size_only)
{
    ...
    if (type->t & VT_ARRAY) {
        ...
        no_oblock = 1;
        if ((first && tcc_state->tok != TOK_LSTR && tcc_state->tok != TOK_STR) || 
            tcc_state->tok == '{') {
            skip('{');
            no_oblock = 0;
        }
        ...
        if ((tcc_state->tok == TOK_LSTR && 
#ifdef TCC_TARGET_PE
             (t1->t & VT_BTYPE) == VT_SHORT && (t1->t & VT_UNSIGNED)
#else
             (t1->t & VT_BTYPE) == VT_INT
#endif
            ) || (tcc_state->tok == TOK_STR && (t1->t & VT_BTYPE) == VT_BYTE)) {
            while (tcc_state->tok == TOK_STR || tcc_state->tok == TOK_LSTR) {
                int cstr_len, ch;
                CString *cstr;

                cstr = tcc_state->tokc.cstr;
                ...
                if (!size_only) {
                    ...
                    /* in order to go faster for common case (char
                       ...
                       specifically */
                    if (sec && tcc_state->tok == TOK_STR && size1 == 1) {
        ...
        parlevel = 0;
        while ((parlevel > 0 || (tcc_state->tok != '}' && tcc_state->tok != ',')) && 
               tcc_state->tok != -1) {
            if (tcc_state->tok == '(')
                parlevel++;
            else if (tcc_state->tok == ')')
                                   ...
                                   int has_init, int v, int scope)
{
    ...
    if (size < 0) {
        ...
        if (has_init == 2) {
            ...
            level = 0;
            while (level > 0 || (tcc_state->tok != ',' && tcc_state->tok != ';')) {
                if (tcc_state->tok < 0)
    ...
    if ((r & VT_VALMASK) == VT_LOCAL) {
        ...
        if (do_bounds_check) {
            ...
            greloc(tcc_state->bounds_section, sym, tcc_state->bounds_section->data_offset, R_DATA_32);
...
static void func_decl_list(Sym *func_sym)
{
    ...
    while (tcc_state->tok != '{' && tcc_state->tok != ';' && tcc_state->tok != ',' && tcc_state->tok != TOK_EOF) {
...
{
    int saved_nocode_wanted = tcc_state->nocode_wanted;
    tcc_state->nocode_wanted = 0;
    tcc_state->ind = tcc_state->cur_text_section->data_offset;
    /* NOTE: we patch the symbol size later */
    put_extern_sym(sym, tcc_state->cur_text_section, tcc_state->ind, 0);
    tcc_state->funcname = get_tok_str(sym->v, NULL);
    tcc_state->func_ind = tcc_state->ind;
    ...
    sym_pop(&tcc_state->local_stack, NULL); /* reset local stack */
    /* end of function */
    /* patch symbol size */
    ((Elf32_Sym *)tcc_state->symtab_section->data)[sym->c].st_size = 
        tcc_state->ind - tcc_state->func_ind;
    tcc_state->funcname = ""; /* for safety */
    tcc_state->func_vt.t = VT_VOID; /* for safety */
    tcc_state->ind = 0; /* for safety */
    tcc_state->nocode_wanted = saved_nocode_wanted;
}
...
static void gen_inline_functions(void)
{
    ...
    for(;;) {
        inline_generated = 0;
        for(sym = tcc_state->global_stack; sym != NULL; sym = sym->prev) {
            type = &sym->type;
            if (((type->t & VT_BTYPE) == VT_FUNC) &&
                ...
                tcc_state->macro_ptr = str;
                next();
                tcc_state->cur_text_section = tcc_state->text_section;
                gen_function(sym);
                tcc_state->macro_ptr = NULL; /* fail safe */
...
static void decl(int l)
{
    ...
    while (1) {
        if (!parse_btype(&btype, &ad)) {
            ...
            if (l == VT_CONST &&
                (tcc_state->tok == TOK_ASM1 || tcc_state->tok == TOK_ASM2 || tcc_state->tok == TOK_ASM3)) {
                /* global asm block */
                asm_global_instr();
                continue;
            }
            ...
            if (l == VT_LOCAL || tcc_state->tok < TOK_DEFINE)
                break;
            btype.t = VT_INT;
        }
        if (((btype.t & VT_BTYPE) == VT_ENUM ||
...
int tcc_relocate(TCCState *s1)
{
    ...
    for(i = 1; i < s1->nb_sections; i++) {
        ...
        if (s->sh_flags & SHF_ALLOC) {
            if (s->sh_type == SHT_NOBITS && s->data == NULL)
...
TCCState *tcc_new(void)
{
    ...
    /* add all tokens */
    s->table_ident = NULL;
    memset(s->hash_ident, 0, TOK_HASH_SIZE * sizeof(TokenSym *));
    ...
    /* Ignore GCC specific features */
    tcc_define_symbol(s, "__asm(x)", "");
    tcc_define_symbol(s, "__aligned(x)", "");

#ifdef _WIN32
    tcc_define_symbol(s, "CONST", "");
#endif
    ...
    s->symtab_section = new_symtab(s, ".symtab", SHT_SYMTAB, 0,
                                   ".strtab",
                                   ".hashtab", SHF_PRIVATE); 
    s->strtab_section = s->symtab_section->link;
...
/* Added by KOGURO, Naoki */
void tcc_state_stack_push(TCCState *s)
{
    struct TCCStateStack *prev = tcc_state_stack;
    tcc_state_stack = tcc_malloc(sizeof(struct TCCStateStack));
    tcc_state_stack->state = s;
    tcc_state_stack->prev = prev;
}
...
void tcc_delete(TCCState *s1)
{
    ...
    /* free hash of symtab_section */
    free_section(s1->symtab_section->hash);
...
{
    add_elf_sym(s->symtab_section, val, 0, 
