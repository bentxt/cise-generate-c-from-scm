...
{
    if (r & VT_SYM)
        greloc(tcc_state->cur_text_section, sym, tcc_state->ind, R_386_32);
    gen_le32(c);
}
...
static void gcall_or_jmp(int is_jmp)
{
    ...
    if ((vtop->r & (VT_VALMASK | VT_LVAL)) == VT_CONST) {
        ...
        if (vtop->r & VT_SYM) {
            /* relocation case */
            greloc(tcc_state->cur_text_section, vtop->sym, 
                   tcc_state->ind + 1, R_386_PC32);
        } else {
            /* put an empty PC32 relocation */
            put_elf_reloc(tcc_state->symtab_section, tcc_state->cur_text_section, 
...
void gfunc_prolog(CType *func_type)
{
    ...
    while ((sym = sym->next) != NULL) {
        ...
        if (param_index < fastcall_nb_regs) {
            ...
            tcc_state->loc -= 4;
            o(0x89);     /* movl */
            gen_modrm(fastcall_regs_ptr[param_index], VT_LOCAL, NULL, tcc_state->loc);
...
void gfunc_epilog(void)
{
    ...
    if (do_bounds_check && func_bound_offset != tcc_state->lbounds_section->data_offset) {
        int saved_ind;
        int *bounds_ptr;
        Sym *sym, *sym_data;
        ...
        *bounds_ptr = 0;
        ...
        greloc(tcc_state->cur_text_section, sym, 
               tcc_state->ind + 1, R_386_PC32);
        oad(0xe8, -4);
    ...
    if (v >= 4096) {
        ...
        greloc(tcc_state->cur_text_section, sym, tcc_state->ind-4, R_386_PC32);
    } else
#endif
    {
...
void gen_bounded_ptr_add(void)
{
    ...
    vtop->c.ul = (tcc_state->cur_text_section->reloc->data_offset - sizeof(Elf32_Rel)); 
}
...
void gen_bounded_ptr_deref(void)
{
    ...
    rel = (Elf32_Rel *)(tcc_state->cur_text_section->reloc->data + vtop->c.ul);
