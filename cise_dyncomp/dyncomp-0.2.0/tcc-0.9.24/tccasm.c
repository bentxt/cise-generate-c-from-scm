...
static void asm_expr_unary(TCCState *s1, ExprValue *pe)
{
    ...
    default:
        if (s1->tok >= TOK_IDENT) {
            ...
            if (!sym) {
                sym = label_push(&s1->asm_labels, s1->tok, 0);
            ...
            tcc_error("bad expression syntax [%s]", get_tok_str(s1->tok, &s1->tokc));
        }
        break;
    }
}
...
{
    asm_new_label1(s1, label, is_local, s1->cur_text_section->sh_num, s1->ind);
}
...
static void asm_free_labels(TCCState *st)
{
    ...
    for(s = st->asm_labels; s != NULL; s = s1) {
        ...
        /* remove label */
        st->table_ident[s->v - TOK_IDENT]->sym_label = NULL;
        sym_free(s);
    }
    st->asm_labels = NULL;
}
...
static void asm_parse_directive(TCCState *s1)
{
    ...
    case TOK_SECTION1:
        {
            ...
            while (s1->tok != ';' && s1->tok != TOK_LINEFEED && s1->tok != ',') {
                if (s1->tok == TOK_STR)
                    pstrcat(sname, sizeof(sname), s1->tokc.cstr->data);
                else
                    pstrcat(sname, sizeof(sname), get_tok_str(s1->tok, NULL));
                next();
            }
            if (s1->tok == ',') {
    ...
    case TOK_ASM_previous:
        ...
        break;
    default:
        tcc_error("unknown assembler directive '.%s'", get_tok_str(s1->tok, NULL));
        break;
    }
}
...
static int tcc_assemble_internal(TCCState *s1, int do_preprocess)
{
    ...
    s1->parse_flags = PARSE_FLAG_ASM_COMMENTS;
    if (do_preprocess)
...
static void tcc_assemble_inline(TCCState *s1, char *str, int len)
{
    ...
    /* same name as current file so that errors are correctly
       reported */
    pstrcpy(bf->filename, sizeof(bf->filename), s1->file->filename);
    bf->line_num = s1->file->line_num;
    saved_file = s1->file;
    s1->file = bf;
    saved_parse_flags = s1->parse_flags;
    saved_macro_ptr = s1->macro_ptr;
    s1->macro_ptr = NULL;
    ...
    if (tcc_state->tok != ':') {
        ...
        for(;;) {
            ...
            if (tcc_state->tok != TOK_STR)
                expect("string constant");
            op->constraint = tcc_malloc(tcc_state->tokc.cstr->size);
...
static void parse_asm_str(CString *astr)
{
    ...
    if (tcc_state->tok != TOK_STR)
        expect("string constant");
    cstr_new(astr);
    while (tcc_state->tok == TOK_STR) {
        /* XXX: add \0 handling too ? */
        cstr_cat(astr, tcc_state->tokc.cstr->data);
        next();
    }
    cstr_ccat(astr, '\0');
}
            ...
            if (tcc_state->tok != ')') {
                ...
                if (tcc_state->tok == ':') {
                    ...
                    for(;;) {
                        if (tcc_state->tok != TOK_STR)
                            expect("string constant");
                        asm_clobber(clobber_regs, tcc_state->tokc.cstr->data);
                        next();
                        if (tcc_state->tok == ',') {
                            next();
                        } else {
